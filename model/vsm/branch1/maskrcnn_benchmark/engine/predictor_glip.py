...
class GLIPDemo(object):
    ...
    def run_on_web_image(self, original_image, original_caption, thresh=0.5):
        predictions = self.compute_prediction(original_image, original_caption)
        top_predictions = self._post_process(predictions, threshold=thresh)

        result = original_image.copy()
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions)
        result, boxes, scores, new_labels = self.overlay_entity_names(result, top_predictions)
        if self.cfg.MODEL.MASK_ON:
            result = self.overlay_mask(result, top_predictions)

        return result, top_predictions, boxes, scores, new_labels
    ...
    def overlay_entity_names(self, image, predictions, names=None):
        ...
        return image, boxes, scores, new_labels
